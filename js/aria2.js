    ...
    init: function(path, onready) {
      ...
      if (jsonrpc_interface.indexOf("http") == 0) {
        ...
        jsonrpc_ws.onerror = function(event) {
          ...
          main_alert("alert-error", "WebSocket错误。您可能需要刷新页面重新启动。");
          ws_callback = {};
        };
    ...
    pause_all: function() {
      ARIA2.request("pauseAll", [],
        function(result) {
          ...
          ARIA2.refresh();
          main_alert("alert-info", "已暂停所有任务。请等待断开BT服务器。", 2000);
        }
      );
    },
    ...
    unpause_all: function() {
      ARIA2.request("unpauseAll", [],
        function(result) {
          ...
          ARIA2.refresh();
          main_alert("alert-info", "已恢复所有任务", 2000);
        }
      );
    },
